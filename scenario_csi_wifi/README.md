...
# Step 1: Load MATLAB data → csi_data_python.pkl
python load_matlab_csi_data.py

# Step 2: Augment data → csi_data_augmented_pytorch.pkl
python create_augmented_dataset.py --difficulty hard

# Step 3: Prepare train/val/test splits → lstm_data_prepared_pytorch.pkl
python prepare_lstm_data_pytorch.py

# Step 4: Train LSTM → lstm_results_pytorch/best_model.pth
python train_csi_lstm_pytorch.py

# Step 5: Create target domain with L-inf shift → csi_data_target_domain.pkl
python create_target_domain.py --severity hard --method all

# Step 6: Evaluate robustness → domain_shift_results/
python test_on_target_domain.py
```

...
def run(cfg, mode, ckpt_path):
    ...
    for l in range(cfg.nr_of_iterations_nn):
        ...
        for b in range(B):
            ...
            for i in range(K_model):
                ...
                for j in range(K_model):
                    temp += tf.reduce_sum(
                        (tf.matmul(tf.transpose(channel_input[b, i, :, :]), V_present[b, j, :, :])) ** 2
                    )
        ...
        receiver_precoder_temp = tf.matmul(tf.transpose(channel_input, perm=[0, 1, 3, 2]), V_present)
        receiver_precoder = tf.divide(receiver_precoder_temp, user_interference_exp2)

        self_interference = tf.reduce_sum(
            (tf.matmul(tf.transpose(channel_input, perm=[0, 1, 3, 2]), V_present)) ** 2,
            axis=2,
        )
        ...
        for k in range(cfg.pgd_steps):
            V_next, _, _, _ = SupNesterov_acc_PGD_step(
                step_init,
                mom1_init,
                mom2_init,
                f"SN_PGD_L{l}_K{k}",
    ...
    # sum-rate over users, averaged over batch
    WSR_final = wsr_sum / B
    ...
    with tf1.Session() as sess:
        ...
        if mode in ["train", "train_then_infer"]:
            ...
            np.savez(
                os.path.join(ckpt_dir, "sn_unfolded_training_log.npz"),
            ...
            plt.title("Super-Nesterov Unfolded WMMSE training curve")
            plt.legend()
            plt.grid(True)
            plt.tight_layout()
            plt.savefig(os.path.join(ckpt_dir, "sn_unfolded_training_curve.png"), dpi=150)
            plt.close()
